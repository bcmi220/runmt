...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('RATv1-%s-%s(%s+%s)' % (l1, l3, l1, l2), []) for l1, l2, l3 in params.rat_steps] +
            [('RATv1-%s-%s(%s+%s)' % (l2, l3, l1, l2), []) for l1, l2, l3 in params.rat_steps] +
            [('RATv2-%s-%s(%s+%s)' % (l1, l3, l1, l2), []) for l1, l2, l3 in params.rat_steps] +
            [('RATv2-%s-%s(%s+%s)' % (l2, l3, l1, l2), []) for l1, l2, l3 in params.rat_steps] +
            [('XBT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.xbt_steps] +
            [('RABT-%s(%s+%s)-%s' % (l3, l1, l2, l1), []) for l1, l2, l3 in params.rabt_steps] +
            [('RABT-%s(%s+%s)-%s' % (l3, l1, l2, l2), []) for l1, l2, l3 in params.rabt_steps] 
        )
    ...
    def get_iterator(self, iter_name, lang1, lang2, stream, pseudo=False):
        """
        ...
        if lang2 is None:
            ...
            if pseudo is True:
                iterator = self.data['ppara'][(_lang1, _lang2)].get_iterator(
                    shuffle=True,
                    group_by_size=self.params.group_by_size,
                    n_sentences=-1,
                )
    ...
    def get_batch(self, iter_name, lang1, lang2=None, stream=False, pseudo=False):
        """
        ...
        if iterator is None:
            iterator = self.get_iterator(iter_name, lang1, lang2, stream, pseudo)
        try:
            x = next(iterator)
    ...
    def mask_out(self, x, lengths):
        ...
        # whole word masking
        # if params.wwm:
        #     id2word = self.data['dico'].id2word
        #     tokens = x.transpose(1, 0).numpy()
        #     for bidx in range(bs):
        #         word_idx = 0
        #         word_indices = []
        #         for sidx in range(slen):
        #             if not id2word[tokens[bidx, sidx]].startswith('##'):
        #                 word_idx += 1
        #             word_indices.append(word_idx)
        #         word_indices = torch.LongTensor(word_indices)
        #         masked_word_indices = word_indices.masked_select(pred_mask[:, bidx] == 1) # slen
        #         masked_word_indices = masked_word_indices.unsqueeze(dim=-1).expand((slen, slen)) # slen x slen
        #         pred_mask[:, bidx] = torch.sum(masked_word_indices == word_indices, dim=0)
    ...
    def reload_checkpoint(self):
        ...
        if not self.params.reset_stopping_criterion:
            self.best_metrics = data['best_metrics']
            self.best_stopping_criterion = data['best_stopping_criterion']
        else:
            logger.warning("Reset stopping criterion.")
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        noises = langs.clone().fill_(0) # 0 for real sequence

        # forward / loss
        tensor = model('fwd', x=x, lengths=lengths, langs=langs, noises=noises, causal=True)
    ...
    def mlm_step(self, lang1, lang2, lambda_coeff):
        ...
        # FIXME: if use seperate position embedding, cannot support TLM now
        if params.use_sep_pos_emb and lang2 is not None:
            return
        ...
        # forward / loss
        tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, noises=noises, causal=False)
        _, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=False)
    ...
    def pc_step(self, lang1, lang2, lambda_coeff):
        ...
        h = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, noises=noises, causal=False)[0]
    ...
    def ppc_step(self, lang1, lang2, lambda_coeff):
        """
        Pseudo Parallel classification step. Predict if pairs of sentences are manual-aligned or pseudo pair.
        """
        pass
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        if lang1 == lang2:
            noises1 = langs1.clone().fill_(1) # 1 for noisy sequence
        else:
            noises1 = langs1.clone().fill_(0) # 0 for real sequence
        noises2 = langs2.clone().fill_(0) # 0 for real sequence

        # encode source sentence
        enc1 = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=False)
        enc1 = enc1.transpose(0, 1)

        # decode target sentence
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, noises=noises2, causal=True, src_enc=enc1, src_len=len1)
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        with torch.no_grad():
            ...
            enc1 = _encoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=False)
            enc1 = enc1.transpose(0, 1)
            x2, len2 = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5))
            langs2 = x2.clone().fill_(lang2_id)
            noises2 = langs2.clone().fill_(1) # 1 for noisy sequence
        ...
        enc2 = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, noises=noises2, causal=False)
        ...
        dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=True, src_enc=enc2, src_len=len2)
    ...
    def mass_step(self, lang, lambda_coeff):
        ...
        # mass has make the sentence incomplete
        noises1 = langs1.clone().fill_(1) # 1 for noisy sequence
        noises2 = langs2.clone().fill_(1) # 1 for normal sequence
        ...
        dec2 = self.decoder('fwd', 
                            x=x2, lengths=len2, langs=langs2, noises=noises2, causal=True, 
    ...
    def rat_step(self, lang1, lang2, lang3, lambda_coeff):
        """
        Pivot Agreement Back-translation step for machine translation.
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        assert lang1 != lang3 and lang1 != lang2 and lang2 != lang3

        params = self.params

        assert not params.asm, 'asm is not supported in rabt step'

        lang1_id = params.lang2id[lang1]
        lang2_id = params.lang2id[lang2]
        lang3_id = params.lang2id[lang3]

        (x1, len1), (x2, len2) = self.get_batch('pat', lang1, lang2)
        ...
        noises1 = langs1.clone().fill_(0) # 0 for normal sequence
        noises2 = langs2.clone().fill_(0) # 0 for normal sequence

        bs = x1.size(1)

        pivot_gen_len = ( ( (len1.float() * 1.3 + 5) + (len2.float() * 1.3 + 5) ) / 2 ).long()
        
        max_len = pivot_gen_len.max().item()

        gen_mask = torch.arange(0, max_len).unsqueeze(0).repeat(bs,1).type_as(pivot_gen_len).lt(pivot_gen_len.unsqueeze(1)) # (bs, max_len)

        # encode source sentence and translate it
        enc1 = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=False)
        enc1 = enc1.transpose(0, 1)
        p_x1 = self.decoder('generate_scores', src_enc=enc1, src_len=len1, tgt_lang_id=lang3_id, max_len=max_len) # (bs, max_len, n_words)

        enc2 = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, noises=noises2, causal=False)
        enc2 = enc2.transpose(0, 1)
        p_x2 = self.decoder('generate_scores', src_enc=enc2, src_len=len2, tgt_lang_id=lang3_id, max_len=max_len) # (bs, max_len, n_words)

        assert p_x1.size() == p_x2.size() and p_x1.size() == (bs, max_len, params.n_words)

        # using jensen shannon (JS)

        kl_loss = nn.KLDivLoss(reduction='none')

        p_both = torch.cat([p_x1.unsqueeze(0), p_x2.unsqueeze(0)], dim=0) # (2, bs, max_len, n_words)
        
        p_mixture = p_both.mean(0, keepdim=True).expand(p_both.size()) # (2, bs, max_len, n_words)

        loss = kl_loss(torch.log(p_both), p_mixture).mean(0) # (bs, max_len, n_words)

        loss = loss * gen_mask.unsqueeze(2).float()

        loss = loss.sum() / pivot_gen_len.sum()

        self.stats[('PAT-%s-%s|%s' % (lang1, lang2, lang3))].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def rat_stepv1(self, lang1, lang2, lang3, lambda_coeff, smoothing_epsilon):
        """
        Pivot Agreement Translation step for machine translation.
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        ...
        (x1, len1), (x2, len2) = self.get_batch('patv1', lang1, lang2)
        ...
        with torch.no_grad():
            ...
            # encode lang1 source sentence and translate it
            enc1 = _encoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=False)
            enc1 = enc1.transpose(0, 1)

            # encode lang2 source sentence and translate it
            enc2 = _encoder('fwd', x=x2, lengths=len2, langs=langs2, noises=noises2, causal=False)
            enc2 = enc2.transpose(0, 1)

            alen3 = ( ( (len1.float() * 1.3 + 5) + (len2.float() * 1.3 + 5) ) / 2 ).long()

            x3, len3 = _decoder.agreement_generate(enc1, len1, enc2, len2, lang3_id, max_len=alen3.max().item())
            langs3 = x3.clone().fill_(lang3_id)
            noises3 = langs3.clone().fill_(1) # 1 for noisy sequence
        ...
        # decode target sentence
        dec3 = self.decoder('fwd', x=x3, lengths=len3, langs=langs3, noises=noises3, causal=True, src_enc=enc1, src_len=len1)

        # loss
        _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y, get_scores=False, smoothing_epsilon=smoothing_epsilon)
        self.stats['PATv1-%s-%s(%s+%s)' % (lang1, lang3, lang1, lang2)].append(loss.item())

        loss1 = lambda_coeff * loss
        ...
        # decode target sentence
        dec3 = self.decoder('fwd', x=x3, lengths=len3, langs=langs3, noises=noises3, causal=True, src_enc=enc2, src_len=len2)
        
        # loss
        _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y, get_scores=False, smoothing_epsilon=smoothing_epsilon)
        self.stats['PATv1-%s-%s(%s+%s)' % (lang2, lang3, lang1, lang2)].append(loss.item())

        loss2 = lambda_coeff * loss
        ...
        # number of processed sentences / words
        self.n_sentences += (params.batch_size * 2)
        self.stats['processed_s'] += (len1.size(0) + len2.size(0))
        self.stats['processed_w'] += ((len1 - 1).sum().item() + (len2 - 1).sum().item())

    def rat_stepv2(self, lang1, lang2, lang3, lambda_coeff, smoothing_epsilon):
        """
        Pivot Agreement Translation step for machine translation.
        """
        assert lambda_coeff >= 0
        ...
        (x1, len1), (x2, len2) = self.get_batch('patv2', lang1, lang2)
        ...
        with torch.no_grad():
            ...
            noisy_x3, noisy_len3 = _decoder.agreement_generate(enc1, len1, enc2, len2, lang3_id, max_len=alen3.max().item())
            noisy_langs3 = noisy_x3.clone().fill_(lang3_id)
            noisy_noises3 = noisy_langs3.clone().fill_(1) # 1 for noisy sequence
            ...
            # perform denoising
            # encode source sentence
            noisy_enc3 = _encoder('fwd', x=noisy_x3, lengths=noisy_len3, langs=noisy_langs3, noises=noisy_noises3, causal=False)
            noisy_enc3 = noisy_enc3.transpose(0, 1)

            x3, len3 = _decoder.generate(noisy_enc3, noisy_len3, lang3_id, max_len=alen3.max().item())
            langs3 = x3.clone().fill_(lang3_id)
            noises3 = langs3.clone().fill_(1) # 1 for noisy sequence
        ...
        # loss
        _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y, get_scores=False, smoothing_epsilon=smoothing_epsilon)
        self.stats['PATv2-%s-%s(%s+%s)' % (lang1, lang3, lang1, lang2)].append(loss.item())
        ...
        # loss
        _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y, get_scores=False, smoothing_epsilon=smoothing_epsilon)
        self.stats['PATv2-%s-%s(%s+%s)' % (lang2, lang3, lang1, lang2)].append(loss.item())
    ...
    def xbt_step(self, lang1, lang2, lang3, lambda_coeff):
        """
        Cross Back-translation step for machine translation.
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        ...
        # generate source batch
        (x1, len1), (x3, len3) = self.get_batch('xbt', lang1, lang3)
        langs1 = x1.clone().fill_(lang1_id)
        langs3 = x3.clone().fill_(lang3_id)
        ...
        noises1 = langs1.clone().fill_(0) # 0 for normal sequence
        noises3 = langs3.clone().fill_(0) # 0 for normal sequence
    ...
    def rabt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        (x1, len1), (x2, len2) = self.get_batch('rabt', lang1, lang2)
        ...
        # encode generate sentence
        enc3 = self.encoder('fwd', x=x3, lengths=len3, langs=langs3, noises=noises3, causal=False)
        ...
        # decode original sentence
        dec1 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, noises=noises1, causal=True, src_enc=enc3, src_len=len3)

        # loss
        _, loss = self.decoder('predict', tensor=dec1, pred_mask=pred_mask1, y=y1, get_scores=False)

        self.stats[('RABT-%s(%s+%s)-%s' % (lang3, lang1, lang2, lang1))].append(loss.item())
        ...
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, noises=noises2, causal=True, src_enc=enc3, src_len=len3)

        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask2, y=y2, get_scores=False)

        
        self.stats[('RABT-%s(%s+%s)-%s' % (lang3, lang1, lang2, lang2))].append(loss.item())
