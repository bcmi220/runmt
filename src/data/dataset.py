...
class Dataset(object):
    ...
    def batch_sentences(self, sentences):
        ...
        batch_len = max([len(s) + 2 for s in sentences])
        if batch_len > N_MAX_POSITIONS:
            batch_len = N_MAX_POSITIONS
        lengths = torch.LongTensor([(len(s) + 2) if (len(s) + 2) <= N_MAX_POSITIONS else N_MAX_POSITIONS for s in sentences])
        sent = torch.LongTensor(batch_len, lengths.size(0)).fill_(self.pad_index)

        sent[0] = self.eos_index
        for i, s in enumerate(sentences):
            if lengths[i] > 2:  # if sentence not empty
                if (len(s) + 2) > N_MAX_POSITIONS:
                    s = s[:N_MAX_POSITIONS-2]
