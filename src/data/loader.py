...
def fix_dico_parameters(params, dico):
    n_words = len(dico)
    bos_index = dico.index(BOS_WORD)
    eos_index = dico.index(EOS_WORD)
    pad_index = dico.index(PAD_WORD)
...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # for denoising auto-encoding, unsupervised knowledge compression and online back-translation, we need a non-stream (batched) dataset
...
def load_para_data(params, data):
    ...
    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps + params.rat_para_langs + params.rabt_para_langs + params.xbt_para_langs)
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # for eval blacklist
            if splt in ['valid', 'test'] and (src, tgt) in params.eval_blacklist:
                continue
...
def check_data_params(params):
    ...
    params.rat_steps = [tuple(tuple(s.split('|')[0].split('-') + [s.split('|')[1]])) for s in params.rat_steps.split(',') if len(s) > 0]
    assert all([len(x) == 3 for x in params.rat_steps])
    assert all([l1 in params.langs and l2 in params.langs and l3 in params.langs for l1, l2, l3 in params.rat_steps])
    assert all([l1 != l2 and l1 != l3 and l1 != l2 for l1, l2, l3 in params.rat_steps])
    assert len(params.rat_steps) == len(set(params.rat_steps))
    assert len(params.rat_steps) == 0 or not params.encoder_only
    params.rat_para_langs = [(l1, l2) for l1, l2, _ in params.rat_steps]

    # reference agreement back-translation steps
    params.rabt_steps = [tuple(tuple(s.split('|')[0].split('-') + [s.split('|')[1]])) for s in params.rabt_steps.split(',') if len(s) > 0]
    assert all([len(x) == 3 for x in params.rabt_steps])
    assert all([l1 in params.langs and l2 in params.langs and l3 in params.langs for l1, l2, l3 in params.rabt_steps])
    assert all([l1 != l2 and l1 != l3 and l1 != l2 for l1, l2, l3 in params.rabt_steps])
    assert len(params.rabt_steps) == len(set(params.rabt_steps))
    assert len(params.rabt_steps) == 0 or not params.encoder_only
    params.rabt_para_langs = [(l1, l2) for l1, l2, _ in params.rabt_steps]
    ...
    assert all([l1 != l3 and l1 != l2 and l2 != l3 for l1, l2, l3 in params.xbt_steps])
    assert len(params.xbt_steps) == len(set(params.xbt_steps))
    assert len(params.xbt_steps) == 0 or not params.encoder_only
    params.xbt_para_langs = [(l1, l3) for l1, l2, l3 in params.xbt_steps]

    # evaluate blacklist
    params.eval_blacklist = [tuple(s.split('-')) for s in params.eval_blacklist.split(',') if len(s) > 0]
    assert all([len(x) == 2 for x in params.eval_blacklist])
    assert all([l1 in params.langs and l2 in params.langs for l1, l2 in params.eval_blacklist])
    assert all([l1 != l2 for l1, l2 in params.eval_blacklist])
    assert len(params.eval_blacklist) == len(set(params.eval_blacklist))
    ...
    for ((src, tgt), paths) in params.para_dataset.items():
        for (splt, (p1, p2)) in paths.items():
            if splt in ['valid', 'test'] and (src, tgt) in params.eval_blacklist:
                continue
...
def load_data(params):
    ...
    # pseudo parallel datasets
    load_pseudo_para_data(params, data)
