...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, get_scores=False, smoothing_epsilon=0.0):
        """
        ...
        if self.asm is False:
            ...
            if smoothing_epsilon > 0:
                ...
                nll_loss = -lprobs.gather(dim=-1, index=y.view(-1, 1))
                smooth_loss = -lprobs.sum(dim=-1, keepdim=True)
                nll_loss = nll_loss.squeeze(-1)
                smooth_loss = smooth_loss.squeeze(-1)
                nll_loss = nll_loss.mean()
                smooth_loss = smooth_loss.mean()
                eps_i = smoothing_epsilon / lprobs.size(-1)
                loss = (1. - smoothing_epsilon) * nll_loss + eps_i * smooth_loss
            else:
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        self.use_noisy_emb = getattr(params, 'use_noisy_emb', False)
        self.use_sep_pos_emb = getattr(params, 'use_sep_pos_emb', False)
        ...
        # embeddings
        if self.use_sep_pos_emb:
            self.position_embeddings = nn.ModuleList([Embedding(N_MAX_POSITIONS, self.dim) for _ in range(self.n_langs)])
        else:
        ...
        if self.use_noisy_emb:
            self.noisy_embeddings = Embedding(2, self.dim) # 0 for normal, 1 for noisy
        ...
        self.attention_setting = getattr(params, 'attention_setting', 'default')
        assert self.use_lang_emb or self.attention_setting != 'default'
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, noises=None, cache=None, enc_mask=None):
        """
        Inputs:
            ...
            `noises` LongTensor(slen, bs), containing noisy IDs
        """
        ...
        assert lengths.max().item() <= slen
        assert slen <= N_MAX_POSITIONS, (slen, N_MAX_POSITIONS)
        ...
        if positions is None:
            ...
            positions = torch.arange(slen, out=positions).unsqueeze(0).expand(bs, slen)
        else:
        ...
        if noises is not None:
            assert noises.size() == (slen, bs)
            noises = noises.transpose(0, 1)
        ...
        if self.use_sep_pos_emb:
            pos_embs = []
            lang_ids = langs[:,0].cpu().numpy()
            for batch_idx, lang_id in enumerate(lang_ids):
                pos_embs.append(self.position_embeddings[lang_id](positions[batch_idx,:]).unsqueeze(0))
            pos_embs = torch.cat(pos_embs, dim=0)
            tensor = tensor + pos_embs
        else:
            tensor = tensor + self.position_embeddings(positions).expand_as(tensor)
        if langs is not None and self.use_lang_emb:
            tensor = tensor + self.lang_embeddings(langs)
        if noises is not None and self.use_noisy_emb:
            tensor = tensor + self.noisy_embeddings(noises)
        ...
        for i in range(self.n_layers):
            ...
            if self.is_decoder and src_enc is not None:
                if self.attention_setting == "default":
    ...
    def predict(self, tensor, pred_mask, y, get_scores, smoothing_epsilon=0.0):
        """
        ...
        scores, loss = self.pred_layer(masked_tensor, y, get_scores, smoothing_epsilon)
        return scores, loss
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        # noisy IDs
        noises = langs.clone().fill_(1) # 1 for noise input
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor, cache = self.forward(
                'fwd',
                x=generated[:cur_len],
                lengths=gen_len,
                positions=positions[:cur_len],
                langs=langs[:cur_len],
                noises=noises[:cur_len],
                causal=True,
                src_enc=src_enc,
                src_len=src_len,
                cache=cache
        ...
        if cur_len == max_len:
            generated[-1].masked_fill_(unfinished_sents.bool(), self.eos_index) # unfinished_sents.byte()
    ...
    def generate_scores(self, src_enc, src_len, tgt_lang_id, max_len=80, sample_temperature=None):
        """
        Decode a sentence given initial start.
        `x`:
            - LongTensor(bs, slen)
        ...
        bs = len(src_len)
        assert src_enc.size(0) == bs
        # assert not self.params.asm, 'asm is not supported in rat step'

        # due to the start <eos>
        max_len = max_len + 1
        ...
        while cur_len < max_len:
            ...
            generated_scores.append(F.softmax(scores, dim=-1).unsqueeze(dim=1)) # (bs, 1, n_words)
        ...
        generated_scores = torch.cat(generated_scores, dim=1) # (bs, max_len, n_words)

        return generated_scores

    def agreement_generate(self, src1_enc, src1_len, src2_enc, src2_len, tgt_lang_id, max_len=200, sample_temperature=None):
        """
        Decode a sentence given initial start.
        `x`:
            - LongTensor(bs, slen)
        ...
        # input batch
        bs = len(src1_len)
        assert src1_enc.size(0) == bs and src2_enc.size(0) == bs

        # generated sentences
        generated = src1_len.new(max_len, bs)  # upcoming output
        generated.fill_(self.pad_index)       # fill upcoming ouput with <PAD>
        generated[0].fill_(self.eos_index)    # we use <EOS> for <BOS> everywhere

        # positions
        positions = src1_len.new(max_len).long()
        positions = torch.arange(max_len, out=positions).unsqueeze(1).expand(max_len, bs)

        # language IDs
        langs = src1_len.new(max_len).long().fill_(tgt_lang_id)
        langs = langs.unsqueeze(1).expand(max_len, bs)
        ...
        # current position / max lengths / length of generated sentences / unfinished sentences
        cur_len = 1
        gen_len = src1_len.clone().fill_(1)
        unfinished_sents = src1_len.clone().fill_(1)
        ...
        while cur_len < max_len:
            ...
            assert tensor1.size() == (1, bs, self.dim), (cur_len, max_len, src1_enc.size(), tensor1.size(), (1, bs, self.dim))
            assert tensor2.size() == (1, bs, self.dim), (cur_len, max_len, src2_enc.size(), tensor2.size(), (1, bs, self.dim))

            tensor1 = tensor1.data[-1, :, :].type_as(src1_enc)  # (bs, dim)
            scores1 = self.pred_layer.get_scores(tensor1)      # (bs, n_words)

            tensor2 = tensor2.data[-1, :, :].type_as(src2_enc)  # (bs, dim)
