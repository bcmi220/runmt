...
def build_model(params, dico):
    ...
    if params.encoder_only:
        ...
        if params.reload_model != '':
            ...
            # 在预训练KCNet语言模型的时候模型是整体保存的，因此需要考虑整体加载
            if ',' in params.reload_model:
                ...
                logger.info("Reloading encoder from %s ..." % params.reload_model)
                enc_reload = reloaded['encoder']
                if all([k.startswith('module.') for k in enc_reload.keys()]):
                    enc_reload = {k[len('module.'):]: v for k, v in enc_reload.items()}
                encoder.load_state_dict(enc_reload)

                logger.info("Reloading decoder from %s ..." % params.reload_model)
